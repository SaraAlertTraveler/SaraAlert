...
class ExportJob < ApplicationJob
  ...
  def perform(config)
    ...
    downloads.concat(create_downloads(config, files))

    return if downloads.empty?

    # Sort downloads by filename so that they are grouped together accordingly after batching
    downloads = downloads.sort_by { |download| download[:filename] }
  ...
  def create_downloads(config, files)
    ...
    files&.each do |file|
      ...
      download = create_download(config[:user_id], file[:content], file[:filename], config[:export_type], content_type)
      downloads << { download_id: download.id, filename: file[:filename] }
    end
    downloads
  end
